...
export function AppWrapper({ children }) {
  ...
  useEffect(() => {
    if (JSON.parse(localStorage.getItem("win_tracker_state"))) {
      //checking if there already is a state in localstorage
      dispatch({
        type: "INIT_STORED",
        data: JSON.parse(localStorage.getItem("win_tracker_state")),
  ...
  useEffect(() => {
    if (state !== initialState) {
      localStorage.setItem("win_tracker_state", JSON.stringify(state));
